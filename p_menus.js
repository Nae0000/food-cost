        ...
        return `<div class="menu-list-row ${selectedMenus.has(m.id) ? 'selected-row' : ''}" id="mcard-${m.id}" style="position:relative; padding-left:45px">
          ...
          <div class="menu-list-stats">
            ...
            <div style="text-align:right">
              ...
              ${cost > 0 ? `<div style="font-size:11px;color:var(--warning);margin-top:2px">${_settings.calcMethod === 'markup' ? t('suggested_price_markup') : t('suggested_price')} <strong>${formatPrice(_settings.calcMethod === 'markup' ? cost * 3 : cost / 0.3)}</strong></div>` : ''}
            </div>
  ...
  container.innerHTML = `
    <div class="page-header" style="align-items:flex-start">
      ...
      <div style="display:flex;gap:12px;align-items:center">
        ${viewToggleHtml}
        <button class="btn btn-secondary" onclick="importAirregiCSV()" title="${t('menu_import_airregi')}" style="padding:6px 12px;background:var(--bg);color:var(--text);border-color:var(--border)">
          <span style="font-size:16px;margin-right:6px">📥</span> <span class="nav-label" style="font-size:13px;font-weight:600">Airregi</span>
        </button>
...
window.openMenuModal = function (id = null) {
  ...
  function calcPreview() {
    ...
    if (el) {
      ...
      if (cost > 0) {
        html += `<span style="color:var(--warning);font-size:12px">${_settings.calcMethod === 'markup' ? t('suggested_price_markup') : t('suggested_price')} <strong>${formatPrice(_settings.calcMethod === 'markup' ? cost * 3 : cost / 0.3)}</strong></span>`;
      }
      el.innerHTML = html;
    }
  ...
  function calcSetPreview() {
    ...
    if (el) {
      ...
      if (setCost > 0) {
        html += `<span style="color:var(--warning);font-size:12px">${_settings.calcMethod === 'markup' ? t('suggested_price_markup') : t('suggested_price')} <strong>${formatPrice(_settings.calcMethod === 'markup' ? setCost * 3 : setCost / 0.3)}</strong></span>`;
      }
      el.innerHTML = html;
    }
    const costEl = document.getElementById('mSubMenuCostVal');
